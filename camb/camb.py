...
def run_ini(ini_filename, no_validate=False):
    ...
    change results in_ctypes.PyCSimpleType' almost all cases).
...
def write_ini(params, filename):
    ...
    write_inifile = camblib.__camb_MOD_camb_writeinifile
    write_inifile.argtypes = [POINTER(CAMBparams), \
                            ctypes.c_char_p, \
                            POINTER(ctypes.c_long)]
    write_inifile.restype = ctypes.c_bool
    s = ctypes.create_string_buffer(filename.encode("latin-1"))
    if not write_inifile(params, s, ctypes.c_long(len(filename))):
        print('Error writing file')

    # there is not a native F function so I will use Python 
    # to write the file at least ads an initila test
    with open(filename, 'w') as f:
        f.write('### Parameters for CAMB generated by python\n')
        print(type(params))
        for v in params._fields_:
            print(type(v), len(v))
            sys.stdout.flush()
