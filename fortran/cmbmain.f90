...
101         if (CP%WantTransfer.and.itf <= State%num_transfer_redshifts) then
    ...
    if (tDissipative<Statein%TimeSteps%points(1)) then
       ...
       nDissipative = statbesseindexof (datasbin%s_count, datasbin%s_R, &
...
!   !     timestep
!
!!   bes_ix_check(1) = datasb%iv_q
!!   bes_ix_check(2) = datasb%s_tau0
!!   do j=1,datasb%iv_sourcessteps
!!       bes_ix_check(j+2) = datasb%s_points(j)
!!   end do
